        ...
        // ===== DASHBOARD DATA LOADING =====
            async function loadDashboardData() {
                try {
                    ...
                    const statsResponse = await fetch('http://localhost:3000/admin/dashboard/stats');
                    ...
                    const activityResponse = await fetch('http://localhost:3000/admin/dashboard/activity');
                    ...
                    if (activities.length === 0) {
                        ...
                        row.innerHTML = '<td colspan="4" class="text-center">No activity today</td>';
            ...
            // Helper function to format timestamp to "X time ago" format
            ...
            // Optional: Add a notification function if you don't already have one
            ...
            `;
                ...
                <td>${request.approved_credits !== null ? request.approved_credits : '-'}</td>
        ...
        // Handle credit approval form submission
            document.getElementById('creditApprovalForm').addEventListener('submit', async (event) => {
                ...
                try {
                    const response = await fetch('http://localhost:3000/admin/credits/approve', {
                        method: 'POST',
                        headers: {
    ...
    `;
        ...
        async function loadUsers() {
                ...
                <td class="action-btn-group">
                    <button class="btn btn-primary" onclick="showEditUserModal(${user.id}, '${user.username}', '${user.email}', ${user.credits}, '${user.role}')">Edit</button>
                    <button class="btn btn-${user.status === 'active' ? 'warning' : 'success'}" 
                            onclick="toggleUserStatus(${user.id})">
                        ${user.status === 'active' ? 'Deactivate' : 'Activate'}
                    </button>
                    <button class="btn btn-danger" onclick="deleteUser(${user.id})">Delete</button>
                    ${user.role === 'user' ? `<button class="btn btn-primary" onclick="promoteToAdmin(${user.id})">Make Admin</button>` : ''}
                </td>
            `;
        ...
        // Updated user form submit handler
            document.getElementById('userForm').addEventListener('submit', async (event) => {
                ...
                async function toggleUserStatus(userId) {
                        try {
                            const response = await fetch(`http://localhost:3000/admin/users/${userId}/toggle-status`, {
                                method: 'POST'
                            });
    ...
    `).join("");
        ...
        async function loadAdmins() {
                ...
                <td class="action-btn-group">
                    <button class="btn btn-success" onclick="approveAdmin(${admin.id})">Approve</button>
                    <button class="btn btn-danger" onclick="rejectAdmin(${admin.id})">Reject</button>
                </td>
            `;
                    ...
                    data.currentAdmins.forEach(admin => {
                        const row = document.createElement('tr');
                        const statusClass = admin.approved === 1 ? 'badge-success' : 'badge-danger';
                ...
                <td class="action-btn-group">
                    <button class="btn btn-warning" onclick="toggleAdminStatus(${admin.id}, ${admin.approved})">
                        ${admin.approved === 1 ? 'Deactivate' : 'Activate'}
                    </button>
                    <button class="btn btn-danger" onclick="removeAdmin(${admin.id})">Remove</button>
                </td>
            `;
            ...
            async function rejectAdmin(adminId) {
                    ...
                    try {
                        const response = await fetch(`http://localhost:3000/admin/reject/${adminId}`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            }
                ...
                async function toggleAdminStatus(adminId, currentStatus) {
                        ...
                        try {
                            const response = await fetch(`http://localhost:3000/admin/toggle-status/${adminId}`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                                }
                    ...
                    async function removeAdmin(adminId) {
                            ...
                            try {
                                const response = await fetch(`http://localhost:3000/admin/remove/${adminId}`, {
                                    method: 'DELETE',
                                    headers: {
                                        'Content-Type': 'application/json'
                                    }
                    ...
                    function searchAdmins() {
                            ...
                            const rows = document.getElementById('adminsTable').getElementsByTagName('tr');
    ...
    document.addEventListener('DOMContentLoaded', loadAdmins);
                    // Load credit requests when the tab is shown
                        document.querySelector('[data-tab="credit-requests"]').addEventListener('click', loadCreditRequests);
