    ...
    `;
        ...
        `;
            ...
            async function loadCreditRequests() {
                try {
                    const user = JSON.parse(localStorage.getItem('user'));
                    const response = await fetch(`http://localhost:3000/credits/requests/${user.email}`);
                    ...
                    <div class="empty-state">
                        ...
                        <div class="request-details">
                            <div class="credits-requested">
                                <h4><i class="fas fa-coins"></i> ${request.credits} Credits Requested</h4>
                            </div>
            ...
            async function updateUserCredits() {
                try {
                    const user = JSON.parse(localStorage.getItem('user'));
                    const response = await fetch(`http://localhost:3000/credits/balance/${user.email}`);
            ...
            async function loadScanHistory() {
                ...
                try {
                    const response = await fetch(`http://localhost:3000/assignment/history/${user.email}`);
        ...
        <div class="history-details">
            ...
            <span class="scan-date">${new Date(scan.scanDate).toLocaleDateString()}</span>
        ...
        <button class="view-doc-btn" onclick="viewDocument('${scan.documentId}')">
            View
        </button>`;
                        ...
                        try {
                            const docResponse = await fetch(`http://localhost:3000/assignment/document/${scan.documentId}?email=${user.email}`);
                            if (docResponse.ok) {
                                const documentData = await docResponse.json();
                                const matchCount = documentData.matches ? documentData.matches.length : 0;

                                // Update the match count display
                                const matchCountElement = document.getElementById(`match-count-${scan.documentId}`);
                                if (matchCountElement) {
                                    matchCountElement.textContent = `${matchCount} similar document${matchCount === 1 ? '' : 's'} found`;
                                }
                            }
                        } catch (error) {
                            console.error(`Error fetching match count for document ${scan.documentId}:`, error);
                            const matchCountElement = document.getElementById(`match-count-${scan.documentId}`);
                            if (matchCountElement) {
            ...
            function displayMatchResults(matches) {
                    console.log("displayMatchResults called with", matches ? matches.length : 0, "matches");
                    ...
                    heading.textContent = `Matching Similarity: ${matches.length} Document${matches.length !== 1 ? 's' : ''} Found`;
                    ...
                    matches.forEach(match => {
                        ...
                        docName.textContent = `Document : ${match.fileName || match.documentId || 'Unnamed Document'}`;
    ...
    async function submitCreditRequest(event) {
        ...
        try {
            ...
            showCustomAlert(`Error submitting credit request: ${error.message}`, 'error');
            console.error(error);
        }
            ...
            // Modify the processFileUpload function to prevent page navigation
                async function processFileUpload(file) {
                    ...
                    if (!user || user.credits < 1) {
                        showCustomAlert('You have insufficient credits to perform a document scan.', 'error');
                        return;
                    }
    ...
    `;
                    ...
                    try {
                        ...
                        showCustomAlert(`Scan complete! Found ${result.matches ? result.matches.length : 0} matches`, 'success');
            ...
            <p style="color: var(--danger-color);">${error.message || 'Error scanning document'}</p>
        ...
        `;
            ...
            function triggerFileUpload() {
                ...
                fileInput.onchange = function () {
                    ...
                    if (user.credits <= 0) {
                        showCustomAlert('You have insufficient credits to perform a document scan. Please request additional credits.', 'error');
            ...
            async function viewDocument(documentId) {
                try {
                    const user = JSON.parse(localStorage.getItem('user'));
                    const response = await fetch(`http://localhost:3000/assignment/document/${documentId}?email=${user.email}`);
                    ...
                    if (response.ok) {
                        ...
                        const documentName = documentData.file_name ||
                            documentData.name ||
                            documentData.title ||
                        ...
                        if (documentData.matches && documentData.matches.length > 0) {
                            ...
                            documentData.matches.forEach(match => {
                                ...
                                <div class="common-topics">
                                    <h4>Common Topics:</h4>
                                    <ul>
                                        ${match.commonTopics.map(topic => `<li>${topic}</li>`).join('')}
                                    </ul>
                                </div>
                            `;
                                }
                                ...
                                <div class="matching-passages">
                                    ...
                                    <div class="passages-list">
                                        ${match.matchingPassages.map(passage =>
                                ...
                                <div class="match-header">
                                    ...
                                    <div class="similarity-indicator">
                                        <div class="similarity-bar">
                                            <div class="bar-fill" style="width: ${match.percentage}%; background-color: ${matchBorderColor}"></div>
                                        </div>
                                        <span class="percentage-value">${match.percentage.toFixed(1)}% similar</span>
                                    </div>
                                </div>
                                ${topicsList}
                                ${passagesList}
                            </div>
                            <button class="view-doc-btn" onclick="viewDocument('${match.documentId}')">
                                View Document
                            </button>
                        `;
